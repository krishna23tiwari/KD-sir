...
exports.trackVisitor = async (req, res) => {
  ...
  try {
    ...
    if (!existingVisitor) {
      const location = await getLocationFromIP(ip); // <-- may be failing here
      await Visitor.create({ ip, date: today, location });

      let counter = await Counter.findOne({ date: today });

      if (!counter) {
        const total = await Counter.findOne().sort({ totalCount: -1 });
        const totalCount = total ? total.totalCount : 0;
    ...
    const todayCounter = await Counter.findOne({ date: today });
    const totalCounter = await Counter.find().sort({ totalCount: -1 }).limit(1);

    return res.status(200).json({
      todayCount: todayCounter?.todayCount || 0,
      totalCount: totalCounter[0]?.totalCount || 0,
    });
  } catch (error) {
