...
const [totalPages, setTotalPages] = useState(1);
  ...
  const fetchAllProjects = async() =>{
    try {
      const res = await axios.get(`${baseurl}projects/getAllprojects`)
  ...
  try {
    const res = await axios.get(`${baseurl}projects/getproject?page=${pageNumber}&limit=3`);
...
const visibleProjects =
  ...
  {/* Pagination buttons — hide when searching */}
  {searchQuery.trim() === "" && (
